    ...
    def process_item(self, item, spider):
        for link in item['links']:
            ...
            if link['title'] == 'Meeting/Agenda Information':
                raw_extracted_text = ''
                """get pdf, open and extract pages (cumulate into single string to store as raw text)"""
                with pdfplumber.open(BytesIO(requests.get(link['href']).content)) as pdf:
                    for page in pdf.pages:
                        raw_extracted_text += page.extract_text()
                        """search for zoom link in first page, store as meeting URL"""
                        if page == pdf.pages[0]:
                            meeting_link = re.search(r"https://(.+?\.)?zoom.us/j/.+?\b", raw_extracted_text, re.M).group()
                            item['location']['url'] = meeting_link if meeting_link else ''
