...
class PostgresPipeline:
    ...
    def from_crawler(cls, crawler):
        return cls(host=crawler.settings.get("POSTGRES_HOST"),
                   user=crawler.settings.get("POSTGRES_USER"),
                   password=crawler.settings.get("POSTGRES_PASSWORD"),
                   database=crawler.settings.get("POSTGRES_DATABASE"))

    def open_spider(self, spider):
        self.conn = psycopg2.connect(host=self.postgres_host,
    ...
    def process_item(self, item, spider):
        data = (item["_id"], item["extras"]["cityscrapers.org/id"], item["name"],
                item["description"], item["classification"], item["status"],
                item["start_time"], item["end_time"], item["timezone"],
                item["all_day"], item["extras"]["cityscrapers.org/time_notes"],
                item["location"]["name"], item["location"]["url"],
        ...
        for link in item["links"]:
            ...
            query = ("SELECT * FROM link WHERE "
                     "meeting_id=%s AND note=%s AND url=%s;")
