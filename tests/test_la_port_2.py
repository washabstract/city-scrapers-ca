...
from city_scrapers.spiders.la_port_2 import LaPortSpider

test_response = file_response(
    join("tests", "files", "la_port_2.html"),
...
test_response_0 = file_response(join("tests", "files", "la_port_2_0.html"), url=parsed_items[0].url)
parsed_items[0] = next(spider._parse_time_location(test_response_0, parsed_items[0]._cb_kwargs["meeting"], parsed_items[0]._cb_kwargs["item"]))
test_response_24 = file_response(join("tests", "files", "la_port_2_24.html"), url=parsed_items[24].url)
parsed_items[24] = next(spider._parse_time_location(test_response_24, parsed_items[24]._cb_kwargs["meeting"], parsed_items[24]._cb_kwargs["item"]))
freezer.stop()
...
def test_id():
    assert parsed_items[0]["id"] == "la_port_2/202201270900/x/regular_board_meeting"
    assert parsed_items[24]["id"] == "la_port_2/202201130900/x/regular_board_meeting"
