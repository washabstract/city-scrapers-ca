...
def test_description():
    ...
    assert parsed_items_jc[0]["description"] == ""
...
def test_id():
    assert (
        parsed_items[0]["id"]
        == "catc/202210120000/x/california_transportation_commission_meeting"
    )
    assert (
        parsed_items_c[0]["id"]
        == "catc/202202250930/x/road_charge_technical_advisory_committee_meeting"
    )
    assert (
        parsed_items_ear[0]["id"]
        == "catc/202202030000/x/equity_advisory_roundtable_meeting_5"
    )
    assert (
        parsed_items_jc[0]["id"]
        == "catc/202211030900/x/joint_ctc_carb_hcd_meeting_riverside"
    )
    assert parsed_items_th[1]["id"] == "catc/202209211300/x/town_hall_meeting"
    assert (
        parsed_items_ts[0]["id"] == "catc/201909160000/x/tri_state_commission_meeting"
    )
    assert (
        parsed_items_w[0]["id"] == "catc/202212131300/x/"
